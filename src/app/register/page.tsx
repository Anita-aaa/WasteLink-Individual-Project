            ...
            <section className="flex min-h-[80vh] justify-center items-center ">
                <div className=" border-2 border-black/10 shadow-md shadow-black/10 w-full m-4 md:m-auto p-4 rounded-lg">
                    ...
                    <form autoComplete="false" className="flex flex-col gap-2" onSubmit={handleRegister}>
                        ...
                        <input type="text" placeholder="Username" className=" bg-transparent border-2 border-back/20 p-2 focus:outline-none focus:border-red-800 duration-200  rounded-lg text-black" onChange={(e) => setUserName(e.target.value)} />
                        <label htmlFor="Email">Email</label>
                        <input type="email" placeholder="Email" className=" bg-transparent border-2 border-black/20 p-2 focus:outline-none focus:border-red-800 duration-200  rounded-lg text-black" onChange={(e) => setEmail(e.target.value)} />
                        <label htmlFor="text">Enter Your State</label>
                        <select onChange={handleStateChange} className="bg-transparent border-2 border-black/20 p-2 focus:outline-none focus:border-red-800 duration-200  rounded-lg text-black">
                        ...
                        <label htmlFor="text">Enter Your District</label>
                        <select onChange={(e) => setCitySelected(e.target.value)} className=" bg-transparent border-2 border-black/20 p-2 focus:outline-none focus:border-red-800 duration-200  rounded-lg text-black">
                        ...
                        {loading ? (
                            <button className=" font-semibold flex gap-3 p-3  bg-red-600 text-white rounded-lg items-center justify-center" disabled={true}>
                                <ClockLoader size={25} color="#fff" />
                                <span>Registering...</span>
                            </button>
                        ) : (
                            <button className={` p-3 ${userName && email && password && confirmPassword != "" && password === confirmPassword ? "bg-red-600 text-white cursor-pointer" : "bg-black/30 text-white cursor-not-allowed"} rounded-lg mt-3 font-semibold duration-200 ${userName && email && password && confirmPassword != "" && password && confirmPassword.length > 8 && password && "hover:bg-geen-500"} hover:text-white`}>Register</button>
                        )}
