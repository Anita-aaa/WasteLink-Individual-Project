            ...
            <section className="flex min-h-[88vh] justify-center items-center">
                <div className=" border-2 border-black/10  shadow-md shadow-black/10 dark:shadow-white/70 w-full  m-4 md:m-auto p-4 rounded-lg">
                    ...
                    <form autoComplete="false" className="flex flex-col gap-2" onSubmit={handleResetPassword}>
                        ...
                        <input type="email" placeholder="Email" onChange={(e) => setEmail(e.target.value)} className=" bg-transparent border-2  border-black/10 p-2 focus:outline-none focus:border-red-600 duration-200  rounded-lg  text-black" autoComplete="off" />
                        {loading ? (
                            ...
                            <button className={` p-3 ${email != "" ? "bg-red-600 text-white cursor-pointer" : "bg-black/30 text-white   cursor-not-allowed"} rounded-lg mt-3 font-semibold duration-200 ${email != "" && "hover:bg-black"} hover:text-white`} disabled={email != "" ? false : true}>
                                Send Email
                            </button>
                        )}
                    </form>
